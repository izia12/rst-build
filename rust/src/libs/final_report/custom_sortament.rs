...
fn find_combinations_for_area_with_limits(
	...
	// Проверяем, есть ли комбинации с отклонением менее 20% И неотрицательным отклонением
	let mut valid_combinations: Vec<(u32, u32, f32)> = result
		.iter()
		.filter(|&&(_, _, area)| area >= target_area && area <= max_area) // Только неотрицательные отклонения
	...
	if valid_combinations.is_empty() {
		...
		// Фильтруем только комбинации с неотрицательными отклонениями
		let positive_combinations: Vec<(u32, u32, f32)> = result
			.iter()
			.filter(|&&(_, _, area)| area >= target_area) // Только неотрицательные отклонения
			.map(|&(d1, d2, area)| (d1, d2, area))
			.collect();
		
		if !positive_combinations.is_empty() {
			// Сортируем по площади (от меньшей к большей)
			let mut sorted_positive = positive_combinations;
			sorted_positive.sort_by(|&(_, _, area_a), &(_, _, area_b)| {
				area_a.partial_cmp(&area_b).unwrap_or(std::cmp::Ordering::Equal)
			});
			
			// Берем первые несколько комбинаций с положительными отклонениями
			valid_combinations = sorted_positive.into_iter().take(3).collect();
		}
		// Если даже положительных комбинаций нет, возвращаем пустой вектор
		// (не добавляем отрицательные отклонения)
	}
