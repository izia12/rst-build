...
pub struct SerializableEntity1 {
...
//     let entities: Vec<SerializableEntity1> = drawing.entities().filter_map(|entity| {
//         match entity.specific {
//             // Handle LINE entities
//             dxf::entities::EntityType::Line(ref line) => Some(SerializableEntity1 {
...
//             dxf::entities::EntityType::Face3D(ref face3d) => Some(SerializableEntity1 {
...
pub fn sli_to_json(data: &str, tolerance: Option<f64>) -> String {
    ...
    for e in parser {
        match e {
            Ok(XmlEvent::StartElement {
                ...
                "Element" => {
                    node_id += 1;
                    let entity_type = match attributes
                        .iter()
                    ...
                    let material_num = attributes
                        ...
                        .parse::<usize>()
                        .unwrap();
                    let entity = SerializableEntity1 {
                        entity_type,
                        vertices: vec![],
                        handle: "".to_string(),
                        layer: "".to_string(),
                ...
                "Nodes" => {
                    let node_indexes = attributes
                        .iter()
                        .map(|attr| attr.value.parse::<usize>().unwrap())
                        .collect::<Vec<usize>>();
    ...
    for entity in entities.iter_mut() {
        let mut coords: Vec<f64> = entity
            .vertices
            .iter()
            .flat_map(|v| vec![v.x, v.y, v.z])
            .collect();
        colored_entities
            .entry(entity.color_id)
            ...
            Ok(XmlEvent::StartElement {
                ...
                match name.local_name.as_str() {
                    ...
                    "Material" => {
                        let num = attributes
                            .iter()
                            .find(|attr| attr.name.local_name == "Num")
                            .unwrap()
                            .value
                            .parse::<usize>()
                            .unwrap();
                        let h = attributes
                            .iter()
                            .find(|attr| attr.name.local_name == "H")
                            .unwrap()
                            .value
                            .parse::<f64>()
                            .unwrap();
...
// #[wasm_bindgen]
pub fn convert_sli_xsl_to_json(
    sli_data: &str,
    txt_data: &str,
    xls_data: &[u8],
) -> Vec<EntityWithXlsx> {
    ...
    for (_, entity) in entities.iter().enumerate() {
        if let Some(row) = xlsx.iter().find(|row_item| row_item.id == entity.node_id) {
            let material = entity
                .material_num
                .and_then(|num| materials.get(&num).cloned());
