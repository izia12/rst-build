...
pub fn generate_color_palette(scale_count: usize) -> Vec<Rgb<u8>> {
    match scale_count {
        ...
        9 => vec![
            ...
            Rgb([197, 44, 0]),    // #C52C00 - седьмой цвет
            Rgb([121, 4, 2]),     // #790402 - восьмой цвет
            Rgb([80, 3, 1]),      // Девятый цвет - темнее последнего
        ],
        10 => vec![
            Rgb([141, 251, 246]), // #8DFBF6 - первый цвет
            Rgb([220, 254, 225]), // #DCFEE1 - второй цвет
            ...
            Rgb([121, 4, 2]),     // #790402 - восьмой цвет
            Rgb([80, 3, 1]),      // Девятый цвет - темнее последнего
            Rgb([50, 2, 1]),      // Десятый цвет - еще темнее
        ],
        11 => vec![
            Rgb([141, 251, 246]), // #8DFBF6 - первый цвет
            Rgb([220, 254, 225]), // #DCFEE1 - второй цвет
        ...
        12 => vec![
            ...
            Rgb([50, 2, 1]),      // Десятый цвет - еще темнее
            Rgb([30, 1, 0]),      // Одиннадцатый цвет
            Rgb([15, 0, 0]),      // Двенадцатый цвет - самый темный
        ],
        _ => {
            if scale_count <= 20 {
...
fn generate_extended_palette(count: usize) -> Vec<Rgb<u8>> {
    // Базовая унифицированная палитра из 12 цветов
    let base_palette = vec![
        Rgb([255, 255, 230]), // Белый с желтым оттенком
        Rgb([175, 238, 238]), // Светлый aqua
        Rgb([64, 224, 208]),  // Темный aqua
        Rgb([255, 255, 102]), // Светло-желтый
        Rgb([255, 255, 0]),   // Желтый
        Rgb([255, 165, 0]),   // Оранжевый
        Rgb([255, 69, 0]),    // Красно-оранжевый
        Rgb([255, 0, 0]),     // Красный
        Rgb([178, 34, 34]),   // Огненно-красный
        Rgb([139, 0, 0]),     // Темно-красный
        Rgb([128, 0, 0]),     // Бордовый
        Rgb([102, 0, 0]),     // Темно-бордовый
    ];
    ...
    if count <= 12 {
        ...
        let additional_colors = vec![
            Rgb([255, 250, 205]), // Лимонный крем
            Rgb([255, 228, 181]), // Мокасин
            Rgb([255, 218, 185]), // Персиковый
            Rgb([255, 160, 122]), // Светло-лососевый
            Rgb([255, 99, 71]),   // Томатный
            Rgb([220, 20, 60]),   // Малиновый
            Rgb([139, 0, 0]),     // Темно-красный
            Rgb([102, 0, 0]),     // Темно-бордовый
        ];
